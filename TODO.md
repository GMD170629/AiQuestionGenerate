...
- [ ] **数据库模型设计**
    - [ ] 设计 `Textbook` 模型：管理教材目录及包含的 Markdown 文件。
    - [ ] 设计 `GenerationTask` 模型：记录任务状态（等待中、执行中、已完成、失败）、总进度、当前文件。
    - [ ] 设计 `Question` 持久化存储：将生成的题目关联至对应的文件和教材。
- [ ] **异步任务处理 (Background Workers)**
    - [ ] 集成 `FastAPI BackgroundTasks` 或 `Celery/Redis`（推荐简单起步用 BackgroundTasks）。
    - [ ] 实现任务调度器：能够按顺序轮询教材下的所有 Markdown 文件，并调用第三阶段的生成逻辑。
- [ ] **实时进度推送**
    - [ ] 实现 `SSE (Server-Sent Events)` 接口，允许前端订阅任务进度更新。

## 📊 第七阶段：自动化全书出题逻辑
- [ ] **智能出题策略优化**
    - [ ] **自适应识别**：修改 `generator.py`，让 AI 先分析切片内容的密度，自主决定生成 3 道还是 10 道题，以及选择最适合该知识点的题型（如算法选编程题，概念选单选题）。
    - [ ] **令牌桶限流控制**：针对全书生成任务，添加频率限制（Rate Limiting）逻辑，防止触发 LLM API 的并发限制。
- [ ] **批量处理流水线**
    - [ ] 实现“教材级”遍历逻辑：读取文件夹 -> 过滤 MD 文件 -> 逐个文件切片 -> 逐个切片调用 AI -> 结果落库。

## 🖥 第八阶段：题目生成管理界面 (Frontend)
- [ ] **任务中心页面**
    - [ ] 开发教材选择器：支持选择已上传的教材文件夹。
    - [ ] 实现任务启动按钮及“后台运行”提示。
- [ ] **可视化进度条**
    - [ ] 使用 `Radix UI` 或 `Shadcn UI` 的 Progress 组件展示总体进度。
    - [ ] 展示详细日志流：如“正在处理：第二章 内存管理.md (75%)...”
- [ ] **任务持久化查看**
    - [ ] 实现任务列表页：即使用户刷新或关闭页面重进，也能从后端获取当前正在执行的任务状态。