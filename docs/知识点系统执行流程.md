...
3. **层级重构**：将知识点分类为三个层级（Level 1/2/3）并建立父子关系
4. **依赖关系构建**：分析知识点之间的学习路径依赖关系
5. **可视化展示**：前端使用 vis-network 展示知识图谱
...
**管理器类**：`KnowledgeGraphManager` (`backend/graph_manager.py:21`)

**全局实例**：`knowledge_graph = KnowledgeGraphManager()` (`backend/graph_manager.py:1297`)
...
| 方法 | 位置 | 说明 |
|------|------|------|
| `get_prerequisite_context()` | `graph_manager.py:185` | 获取前置知识点上下文 |
| `get_concept_info()` | `graph_manager.py:291` | 获取知识点详细信息 |
| `get_all_concepts()` | `graph_manager.py:328` | 获取所有知识点列表 |
| `get_graph_stats()` | `graph_manager.py:340` | 获取图谱统计信息 |
| `find_path()` | `graph_manager.py:358` | 查找知识点之间的路径 |
| `get_topological_order()` | `graph_manager.py:386` | 获取拓扑排序 |
...
**入口函数**：`load_from_database()` (`backend/graph_manager.py:32`)
...
2. 清空映射和元数据（concept_to_node_id, concept_metadata 等）
   ↓
3. 从数据库获取所有知识点节点
   SQL: SELECT * FROM knowledge_nodes ORDER BY level ASC, created_at ASC
   ↓
4. 第一遍：创建所有节点
   - 遍历每个知识点节点
...
**入口函数**：`reconstruct_hierarchy()` (`backend/graph_manager.py:403`)
...
1. 获取教材的所有知识点节点
   ↓
2. 构建 LLM 提示词：
   - System Prompt：层级分类规则和格式要求
   - User Prompt：教材名称 + 知识点列表
   ↓
3. 调用 LLM API 进行分类
   ↓
4. 解析 JSON 响应：
   - level_1: [{concept, children}, ...]
   - level_2: [{concept, parent, children}, ...]
   - level_3: [{concept, parent}, ...]
   ↓
5. 验证和建立父子关系：
   - Level 2 的 parent 必须是 Level 1
   - Level 3 的 parent 必须是 Level 2
   ↓
6. 更新数据库：
   - 更新每个节点的 level 和 parent_id
   ↓
7. 返回统计信息
```
...
```python:backend/graph_manager.py
async def reconstruct_hierarchy(textbook_id: str, ...):
    # 获取所有知识点
    knowledge_nodes = db.get_textbook_knowledge_nodes(textbook_id)
    
    # 调用 LLM 分类
    hierarchy_data = await call_llm_api(...)
    
    # 建立父子关系
    for concept, level in concept_to_level.items():
        parent_id = concept_to_parent.get(concept)
        db.update_knowledge_node_level(concept, level, parent_id)
```
...
1. **Level 1 间依赖**：一级概念间的先后顺序
2. **Level 2 间依赖**：同一父节点下，二级概念的逻辑递进关系
3. **Level 3 间依赖**：具体技术点之间的微观依赖
...
**入口函数**：`build_dependency_edges()` (`backend/graph_manager.py:799`)

**执行流程**：

```
1. 获取教材的所有知识点节点（按层级分组）
   ↓
2. 构建临时图（用于检测循环）
   ↓
3. 分别处理三个层级：
   ...
   c. Level 3 依赖（按父节点分组）：
      ...
      - 添加边并检测循环
   ↓
4. 批量更新到数据库（knowledge_dependencies 表）
   ↓
5. 返回统计信息
```
...
```python:backend/graph_manager.py
async def build_dependency_edges(textbook_id: str, ...):
    # 按层级分组
    level_1_nodes = [n for n in nodes if n.level == 1]
    level_2_nodes = [n for n in nodes if n.level == 2]
    level_3_nodes = [n for n in nodes if n.level == 3]
    
    # 构建临时图
    temp_graph = nx.DiGraph()
    
    # 处理 Level 1
    dependencies = await _analyze_level_dependencies(level_1_concepts, level=1, ...)
    for dep in dependencies:
        temp_graph.add_edge(source_id, target_id)
        if not nx.is_directed_acyclic_graph(temp_graph):
            temp_graph.remove_edge(source_id, target_id)  # 避免循环
        else:
            all_edges.append((source_id, target_id))
    
    # 批量更新数据库
    db.insert_knowledge_dependencies(all_edges)
```

### 3. 依赖关系分析

**辅助函数**：`_analyze_level_dependencies()` (`backend/graph_manager.py:1113`)

**分析原则**：

1. **基于学习路径**：依赖关系应反映学习顺序
2. **避免循环依赖**：不能创建循环依赖
3. **必要性**：只标记真正必要的依赖关系
4. **简洁性**：每个知识点的前置依赖数量控制在 0-3 个
...
使用 NetworkX 的 `is_directed_acyclic_graph()` 方法检测循环：
...
**数据库表**：`knowledge_dependencies`

**存储字段**：

| 字段 | 类型 | 说明 |
|------|------|------|
| source_node_id | TEXT | 源节点 ID（前置知识点） |
| target_node_id | TEXT | 目标节点 ID（依赖知识点） |
| dependency_type | TEXT | 依赖类型（"depends_on"） |
| created_at | TEXT | 创建时间 |
...
2. **知识图谱构建**：从数据库加载 → 构建 NetworkX 图 → 建立依赖关系
3. **层级重构**：调用 LLM 分类 → 建立父子关系 → 更新数据库
4. **依赖关系构建**：按层级分析 → 检测循环 → 存储依赖关系
5. **前端展示**：获取图谱数据 → 可视化渲染 → 交互展示
...
- **去重机制**：提取过程中检查重复知识点
- **循环检测**：依赖关系构建时避免循环依赖
- **层级结构**：三级知识点层级体系（Global → Chapter → Unit）
...
4. **批量操作**：支持批量层级重构和依赖构建
5. **导出功能**：支持导出知识图谱为多种格式
